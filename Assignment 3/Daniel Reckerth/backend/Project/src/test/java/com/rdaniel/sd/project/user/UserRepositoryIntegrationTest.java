...
import static com.rdaniel.sd.project.user.model.RoleType.ROLE_ADMIN;
...
class UserRepositoryIntegrationTest {
  ...
  void findAllByRolesNameIn() {
    ...
    final Role role = Role.builder().name(ROLE_EMPLOYEE).build();
    roleRepository.save(role);
    users.forEach(user -> user.setRoles(Set.of(role)));

    final Role adminRole = Role.builder().name(ROLE_ADMIN).build();
    ...
    final List<User> regularUsersFound = userRepository.findAllByRolesNameIn(Set.of(ROLE_EMPLOYEE));

    int size = users.size() - 1;
    assertEquals(size, regularUsersFound.size());
  }
}