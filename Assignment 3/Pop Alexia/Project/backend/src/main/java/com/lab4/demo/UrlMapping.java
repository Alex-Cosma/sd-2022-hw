package com.lab4.demo;

public class UrlMapping {
    public static final String API_PATH = "/api";
    public static final String QUESTION = API_PATH + "/question";
    public static final String QUIZZ = API_PATH + "/quizz";
    public static final String QUIZZ_SESSION = API_PATH + "/quizzSession";
    public static final String REVIEW = API_PATH + "/review";
    public static final String ANSWER = API_PATH + "/answer";
    public static final String EXPORT_REPORT = "/export/{type}";

    public static final String AUTH = API_PATH + "/auth";
    public static final String SIGN_IN = "/sign-in";
    public static final String SIGN_UP = "/sign-up";

    public static final String USER = API_PATH + "/user";

}
